...
class _DistTestBase(object):
    ...
    def test_all_reduce_multigpu(self):
        group, group_id, rank = self._init_global_test()
        visible_gpus_per_rank = self._init_multigpu_helper()
        self._test_all_reduce_multigpu_helper(group, group_id, rank,
                                              visible_gpus_per_rank,
                                              dist.reduce_op.SUM,
                                              2, 10,
                                              (2 + 10 * (len(group) -1))
                                              * len(visible_gpus_per_rank[0]))
    ...
    def test_reduce_multigpu(self):
        group, group_id, rank = self._init_global_test()
        visible_gpus_per_rank = self._init_multigpu_helper()
        self._test_reduce_multigpu_helper(group, group_id, rank,
                                          visible_gpus_per_rank,
                                          dist.reduce_op.SUM,
                                          2, 10,
                                          (2 + 10 * (len(group) - 1))
    ...
    class TestTCPOrGloo(TestCase, _DistTestBase):
        ...
        def _join_and_reduce(self, fn):
            ...
            if skip_ok:
                ...
                assert first_process.exitcode == 0 or first_process.exitcode == SKIP_IF_NO_CUDA_EXIT_CODE or first_process.exitcode == SKIP_IF_SINGLE_GPU_EXIT_CODE
